...
exports.register=(req,res)=>{
    ...
    db.query(sql,[userInfo.name],function(err,result){
        //执行SQL语句失败
        if(err){
            return res.cc(err)
        }
        ...
        console.log(userInfo)
        db.query(sql,{name:userInfo.name,password:userInfo.password},(err,result)=>{
...
exports.login=(req,res)=>{
    ...
    db.query(sql,userInfo.name,function(err,result){
        ...
        console.log(bcrypt.compareSync(userInfo.password,result[0].password))
        ...
        const tokenStr=jwt.sign(user,config.jwtSecrectKey,{expiresIn:'24h'})
        //将生成的token响应给客户端
        res.send({
            status:0,
            message:'登录成功',
